...
Nodes_info = function( node,taxono,p4,data_out_res){ #This function gets the node name and prints the information for each node: number of leaves, jaccard and 16S mean and taxonomy
  ...
  p4<-p4+
    geom_cladelabel(edge_table[edge_table$par.name==node,1][1], paste(node,". #Leaves: ",length(mini$tip.label),". Jaccard: ",round(mean(values_j),digits = 3),"±",round(sd(values_j),digits = 3),". 16S: ",round(mean(values_16S),digits = 3),"±",round(sd(values_16S),digits = 3),". Taxonomy: ",concat_taxo,sep = ''), barsize=2, offset.text=1.5, hjust=0.5, fontsize=3)
